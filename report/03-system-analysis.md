...
```mermaid
%% {{FIGURE: report/figures/fig-3.1-uc-analysis.mmd — Figure 3.1}}
flowchart LR
    admin(["Admin<br/>(content steward)"])
    teacher(["Teacher<br/>(paper author)"])
    pyservice(["System / Python service<br/>(secondary actor)"])

    subgraph QForge["QForge system boundary"]
        direction TB
        UC1(["Manage users & roles"])
        UC2(["Manage subjects & units"])
        UC3(["Manage question bank"])
        UC4(["Upload past-paper / syllabus PDF"])
        UC5(["Review extracted candidates"])
        UC6(["Import syllabus into catalog"])
        UC7(["Build blueprint"])
        UC8(["Generate paper"])
        UC9(["Expand bank with AI (top-up)"])
        UC10(["Save & edit paper"])
        UC11(["Export paper (PDF / DOCX)"])
        UC12(["View history & analytics"])
        UC13(["Extract candidates from PDF"])
        UC14(["Generate AI questions"])
    end
    ...
    UC4 -. "&laquo;include&raquo;" .-> UC13
    UC6 -. "&laquo;include&raquo;" .-> UC13
    UC9 -. "&laquo;include&raquo;" .-> UC14
...
```mermaid
%% {{FIGURE: report/figures/fig-3.2-gantt-schedule.mmd — Figure 3.2. Dates are {{PLACEHOLDER}} tokens;
%% milestone order is real. This source renders only after the date tokens are filled in.}}
gantt
    title QForge Build Schedule (Milestone-first, M1–M6)
    dateFormat YYYY-MM-DD
    axisFormat %b %Y
    section Foundation
    M1 Domain foundation (schema, CRUD, roles)      :m1, {{PLACEHOLDER:M1_START}}, {{PLACEHOLDER:M1_DAYS}}d
    section Algorithm (centrepiece)
    M2 Generation engine (greedy + backtracking)    :m2, {{PLACEHOLDER:M2_START}}, {{PLACEHOLDER:M2_DAYS}}d
    M3 Papers lifecycle + export + repetition       :m3, {{PLACEHOLDER:M3_START}}, {{PLACEHOLDER:M3_DAYS}}d
    section Ingestion
    M4 PDF extraction pipeline (OCR fallback)       :m4, {{PLACEHOLDER:M4_START}}, {{PLACEHOLDER:M4_DAYS}}d
    M4.1 Syllabus import (subjects + units)         :m41, {{PLACEHOLDER:M41_START}}, {{PLACEHOLDER:M41_DAYS}}d
    section AI augmentation
    M5 AI bank expansion (Ollama top-up)            :m5, {{PLACEHOLDER:M5_START}}, {{PLACEHOLDER:M5_DAYS}}d
    M6 RAG (embeddings, Qdrant, similarity guard)   :m6, {{PLACEHOLDER:M6_START}}, {{PLACEHOLDER:M6_DAYS}}d
```
...
```mermaid
%% {{FIGURE: report/figures/fig-3.3-class-analysis.mmd — Figure 3.3}}
classDiagram
    class User {
        +int id
        +string name
...
```mermaid
%% {{FIGURE: report/figures/fig-3.4-object-snapshot.mmd — Figure 3.4}}
flowchart TB
    subj["cs301 : Subject<br/>code = &quot;CS301&quot;<br/>name = &quot;Data Structures&quot;"]
    bp["midterm : Blueprint<br/>name = &quot;Standard Midterm&quot;<br/>total_marks = 40<br/>ai_assist = false"]
    slotA["slotA : Slot<br/>type = &quot;long&quot;, marks = 10<br/>allowedUnits = &lbrace;U1,U2,U3&rbrace;"]
    slotB["slotB : Slot<br/>type = &quot;short&quot;, marks = 5<br/>allowedUnits = &lbrace;U1,U2,U3&rbrace;"]
    paper["paper : Paper<br/>status = &quot;saved&quot;<br/>origin = &quot;generated&quot;<br/>total_marks = 40"]
    q17["q17 : Question<br/>type = &quot;long&quot;, marks = 10<br/>unit = U2, status = &quot;approved&quot;"]
    q42["q42 : Question<br/>type = &quot;short&quot;, marks = 5<br/>unit = U3, status = &quot;approved&quot;"]
    pq1["pq1 : PaperQuestion<br/>display_no = 1, section = &quot;A&quot;<br/>marks = 10, is_ai = false"]
    pq2["pq2 : PaperQuestion<br/>display_no = 2, section = &quot;B&quot;<br/>marks = 5, is_ai = false"]

    bp -->|targets| subj
    bp -->|compiled into| slotA
    bp -->|compiled into| slotB
    bp -->|generates| paper
    slotA -.->|filled by| q17
    slotB -.->|filled by| q42
    paper -->|contains| pq1
...
```mermaid
%% {{FIGURE: report/figures/fig-3.5-state-question.mmd — Figure 3.5}}
stateDiagram-v2
    [*] --> pending : extracted / manual create
    [*] --> approved : AI top-up (source = ai)
    pending --> approved : admin approves
    pending --> rejected : admin rejects
    approved --> [*] : deleted / re-extracted
    rejected --> [*] : discarded
    note right of approved
        Only approved questions are
...
```mermaid
%% {{FIGURE: report/figures/fig-3.6-state-paper.mmd — Figure 3.6}}
stateDiagram-v2
...
```mermaid
%% {{FIGURE: report/figures/fig-3.7-state-upload.mmd — Figure 3.7}}
stateDiagram-v2

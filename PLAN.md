...
| Area | Decision |
|---|---|
| Generation algorithm | **Pure PHP in Laravel, from scratch** — slot creation, candidate filtering, greedy fill, constraint validation, backtracking. Unit-tested, no external solver. Determinism is **seed-parameterised** (post-M6): a null seed reproduces the original id-ordered paper (test-safe), a supplied seed yields a *different* valid paper — so "regenerate" and two teachers no longer collide, while any given seed stays fully reproducible. The controller draws a fresh random seed per request and echoes it in the response. |
...
| Unit coverage strategy | **Rule-based, not scored** (post-M5.1): greedy ranks uncovered-unit-first, then LRU, then a **seeded tie-break** (post-M6 — ascending id when unseeded, a reproducible per-seed shuffle otherwise, so the seed only reorders questions the rank already treats as equal); per-unit maximums are hard caps; repetition control is the last-N-**papers** exclusion window, a last-N-**exam-years** exclusion (post-M6 — `exclusionRules.excludeExamYearsBack`: drop questions whose `attributes.exam_year` falls in the N years before the current one, so a paper won't reuse recent real exams' questions; questions with no recorded year stay eligible), **plus a within-paper semantic-duplicate guard** (post-M6 — a soft, fail-open preference that steers the greedy pass away from placing two questions that *mean* the same thing; never a hard constraint, so it can't cause a shortfall — see RAG-GUIDE); no additive scoring formula, no difficulty/frequency weights. Structural infeasibility = `units > 2×slots` (AI questions span ≤ 2 units) or all-capped `Σcaps < slots`. |
...
- `POST /papers/generate` → body `{ blueprint_id, seed? }` → runs algorithm synchronously → returns an **unsaved preview** (`paper.id: null`) + `seed` + `constraint_results[]`, or `{ satisfiable:false, missing_slots:[...] }`. **Persists nothing** (post-M6 — preview-first). *Save* is the separate `POST /papers` `{ blueprint_id, seed, name? }`, which re-generates deterministically from the seed and persists as `status=saved`, bumping `used_count` + `last_used_at` (422 if the bank changed so the seed no longer satisfies). Lifecycle: preview → saved → exported.
...
- `GreedySelector` — fills each slot picking the best candidate, prioritizing unit balance, least-recently-used (lowest `used_count`/oldest usage), then a seeded tie-break (`Support/TieBreaker` — id order when unseeded, a per-seed shuffle otherwise). `PaperGenerator::greedyFill` additionally *prefers* candidates that aren't near-duplicates of questions already placed, via an injected `SimilarityGuard` (post-M6) — soft and pool-non-emptying, so dedup never blocks a fill.
...
- `PaperGenerator` (facade) — orchestrates the above; returns a `GenerationResult` (success + paper draft, or failure + `missing_slots` describing exactly what the bank lacks, e.g. "2× 10-mark, Unit 3"). Each `MissingSlot` also carries a server-set nullable `unitId` (the generator already knows it) so the M5 expansion job targets an allowed `unit_id` without re-parsing a name — `UnitResolver` is for printed PDF headings, not this. `generate()` takes two optional per-run parameters (post-M6): `?int $seed` (threaded into greedy + backtracking tie-breaks) and `?SimilarityGuard $guard` (within-paper semantic dedup, defaulting to the no-op `NullSimilarityGuard` so the engine stays a pure function of the bank in tests). Similarity is deliberately applied only in the greedy pass — never in backtracking — so a soft preference can never turn a satisfiable paper infeasible.

**Tests** (`tests/Unit/PaperGeneration/`): feasible blueprint → valid paper; unit-coverage enforcement; repetition exclusion across last N papers; infeasible blueprint → correct `missing_slots`; backtracking recovers a solvable-but-greedy-fails case; **seeded variation** (same seed reproduces a paper, different seeds vary it, no seed preserves the legacy id order — `TieBreakerTest` + `PaperGeneratorTest`); **within-paper dedup** (greedy avoids a near-duplicate when an alternative exists, never causes a shortfall otherwise; guard fails open when RAG is down — `VectorSimilarityGuardTest`). These tests *are* the proof of the academic contribution.
